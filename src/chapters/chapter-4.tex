...
Ketiga, seberapa efektif LQR dibandingkan kontroler lain (berbasis aturan, PID, dan ANN
berbasis Boltzmann MLE) dalam menjaga antrean tetap rendah pada berbagai pola beban.
...
Pada masing-masing konfigurasi diuji sembilan strategi pengendalian, terdiri dari tiga
kontroler baseline (Statik, Rule-Based, PID), tiga varian LQR per preset $Q$
(LQR-Q1, LQR-Q2, LQR-Q3), dan tiga varian ANN per preset $Q$ (ANN-Q1, ANN-Q2, ANN-Q3).
LQR dan ANN dilatih dengan target perilaku berbeda meskipun keduanya \emph{Q-aware}.
Arsitektur ANN dibahas pada Subbab~\ref{sec:ann-arch}.
...
sehingga total terdapat 45 percobaan per konfigurasi sumber daya.
Pada tabel perbandingan, baris LQR dan ANN merepresentasikan varian yang dicocokkan dengan
preset $Q$ di kolom tersebut (\emph{matched controller}).

\subsection{Arsitektur Kontroler ANN}
\label{sec:ann-arch}

Sebagai pembanding berbasis data terhadap LQR berbasis model, sebuah \emph{Multi-Layer
Perceptron} (MLP) sederhana dilatih dengan input 8 dimensi (4 variabel state ternormalisasi
ditambah 4 nilai diagonal $Q$ yang ditransformasi $\log_{10}$) dan output 2 dimensi
(\emph{batch\_size} dan \emph{inv\_poll\_interval} ternormalisasi). Arsitekturnya
$8 \rightarrow 128 \rightarrow 128 \rightarrow 2$ dengan aktivasi ReLU pada dua lapisan
tersembunyi.

Metode pelatihannya adalah \emph{Boltzmann MLE} pada simulator yang dibangun dari
\emph{capacity benchmark} sebagai model \emph{throughput}. Untuk setiap pasang (state acak,
$Q$ acak), aksi $u$ optimal disampel dari distribusi Boltzmann
$P(u \mid x, Q) \propto \exp(-J(u)/T)$ dan ANN dilatih untuk memaksimumkan \emph{likelihood}
pemetaan dari kondisi tersebut ke $u$. Pendekatan ini tidak memerlukan demonstrasi LQR
sebagai \emph{oracle}, dan menghasilkan satu model yang dapat menangani seluruh preset $Q$
hanya dengan mengubah input $Q$.

Perbedaan utama ANN dan LQR ada pada sumber pengetahuan. LQR memanfaatkan matriks $A,B$
hasil identifikasi sistem dan menyelesaikan DARE, sementara ANN mempelajari pemetaan
\emph{state-action} dari simulator data-driven. Karena keduanya memakai matriks $Q$ yang
sama, keduanya termasuk \emph{Q-aware} dan dapat dibandingkan dalam satu kerangka biaya.
...
\begin{table}[H]
    ...
    \hline
    \textbf{Kontroler} & \textbf{Q1 (Backlog)} & \textbf{Q2 (Resource)} & \textbf{Q3 (Balanced)} & \textbf{Mean Regret} \\
    \hline
    Statik           & 287{,}10 & 272{,}72 & 447{,}06 & 1{,}099 \\
    \hline
    Rule-Based       & 387{,}24 & 179{,}62 & 452{,}24 & 0{,}919 \\
    \hline
    PID              & 256{,}42 & 125{,}35 & 304{,}75 & 0{,}302 \\
    \hline
    ANN              & 211{,}07 & 120{,}41 & 288{,}46 & 0{,}188 \\
    \hline
    \textbf{LQR}     & \textbf{205{,}27} & \textbf{88{,}65} & \textbf{245{,}27} & \textbf{0{,}000} \\
    \hline
    \end{tabular}
\end{table}

LQR menempati posisi terbaik (regret 0) pada seluruh preset $Q$, dengan ANN sebagai
kontroler adaptif kedua terbaik (regret 0{,}188). Kedua kontroler \emph{Q-aware} ini
mencatat regret lebih rendah dari PID (regret 0{,}302), dan selisihnya terhadap Rule-Based
maupun Statik yang tidak adaptif jauh lebih besar.
Selisih besar terhadap Statik mengonfirmasi bahwa parameter statik tidak mampu memanfaatkan
kapasitas tambahan, sementara LQR dan ANN menyesuaikan aksi kontrol terhadap dinamika
antrean. Posisi ANN yang sedikit di bawah LQR menunjukkan bahwa pendekatan \emph{data-driven}
via Boltzmann MLE mendekati performa kontrol optimal berbasis model, meskipun belum
sepenuhnya menyamai.
...
LQR mencapai \emph{throughput} tertinggi pada pola \emph{impulse}, sehingga drainase
berjalan lebih cepat ketika antrean melonjak ke 10.000 secara mendadak. Pola yang sama
...
Pada \emph{timeseries} antrean, kurva LQR terlihat paling rendah dan paling stabil di pola \emph{step} dan \emph{periodic\_step}, yaitu kondisi di mana antrean menumpuk terus sehingga keterlambatan reaksi kontroler langsung terlihat sebagai akumulasi backlog.
...
\begin{table}[H]
    ...
    \textbf{Kontroler} & \textbf{Q1 (Backlog)} & \textbf{Q2 (Resource)} & \textbf{Q3 (Balanced)} & \textbf{Mean Regret} \\
    \hline
    Statik           & 282{,}21 & 180{,}56 & 369{,}23 & 0{,}751 \\
    \hline
    Rule-Based       & 411{,}94 & 188{,}88 & 479{,}75 & 1{,}203 \\
    \hline
    PID              & 253{,}53 & 114{,}00 & 293{,}49 & 0{,}345 \\
    \hline
    ANN              & 244{,}25 & 126{,}38 & 325{,}18 & 0{,}415 \\
    \hline
    \textbf{LQR}     & \textbf{161{,}22} & \textbf{91{,}72} & \textbf{240{,}72} & \textbf{0{,}000} \\
    \hline
    \end{tabular}
\end{table}

Hierarki kontroler relatif konsisten dengan konfigurasi penuh, dengan LQR di posisi
terbaik (regret 0) diikuti PID (0{,}345) dan ANN (0{,}415). Rule-Based justru memburuk
melebihi Statik karena ambang tetap pada \emph{batch} dan \emph{poll} sudah tidak sesuai
dengan kapasitas yang berkurang. Posisi ANN sedikit di bawah PID pada konfigurasi setengah
mengindikasikan bahwa simulator \emph{capacity benchmark} yang digunakan untuk pelatihan
ANN mencerminkan kondisi penuh sehingga generalisasi ke konfigurasi kapasitas berbeda
...
Tabel~\ref{tab:robustness} merangkum \emph{regret} rata-rata empat kombinasi mesin dan
konfigurasi. LQR selalu di posisi terbaik dengan \emph{regret} mendekati nol di seluruh
skenario, diikuti PID dan ANN yang berada di kelompok kontroler adaptif. Statik dan
Rule-Based mengalami lonjakan \emph{regret} sangat besar pada mesin Intel (hingga 39 kali)
karena karakteristik perangkat keras yang berbeda membuat parameter tetap menjadi sangat
tidak optimal. ANN cenderung lebih sensitif terhadap perbedaan mesin dibanding LQR,
konsisten dengan asumsi simulator pelatihannya yang dikalibrasi pada satu kondisi.

\begin{table}[H]
    \centering
    \caption{Perbandingan \emph{regret} rata-rata pada empat kombinasi mesin dan sumber daya}
    ...
    \hline
    Statik       &  1{,}099 &  0{,}751 & 39{,}410 & 27{,}287 \\
    \hline
    Rule-Based   &  0{,}919 &  1{,}203 & 15{,}875 & 13{,}702 \\
    \hline
    PID          &  0{,}302 &  0{,}345 &  0{,}147 &  0{,}080 \\
    \hline
    ANN          &  0{,}188 &  0{,}415 &  0{,}486 &  0{,}533 \\
    \hline
    \textbf{LQR} & \textbf{0{,}000} & \textbf{0{,}000} & \textbf{0{,}074} & \textbf{0{,}024} \\
    \hline
    \end{tabular}
\end{table}
...
Salah satu karakteristik kontroler \emph{Q-aware} (LQR dan ANN) yang membedakannya dari
kontroler lain adalah kemampuannya mengubah perilaku berdasarkan matriks bobot $Q$.
Gambar~\ref{fig:state-across-q-lqr} menampilkan bagaimana LQR dan ANN menyesuaikan
variabel sistem saat preset $Q$ berubah.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{IV_R4__state_across_q_full.png}
    \caption{Rata-rata variabel sistem LQR dan ANN pada tiga preset $Q$ (sumber daya penuh)}
    \label{fig:state-across-q-lqr}
\end{figure}

LQR maupun ANN menanggapi setiap preset $Q$ dengan menggeser titik kerja sesuai prioritas.
Pada preset Q1 yang mengutamakan antrean, keduanya menarik latensi ke nilai paling rendah.
Pada preset Q2 yang menonjolkan sumber daya, CPU dan operasi I/O turun meskipun latensi
sedikit naik. Q3 berada di antara keduanya. Pola pergeseran ANN tidak identik dengan LQR
karena ANN dilatih dari pemetaan Boltzmann pada simulator, sedangkan LQR menyelesaikan
DARE secara analitik. Meskipun jalur penyesuaian berbeda, keduanya tetap mengikuti arah
prioritas $Q$ yang diberikan, mengonfirmasi bahwa konsep \emph{Q-awareness} bukan eksklusif
pada metode kontrol optimal klasik.
...
Sebaliknya, kelebihan utama LQR sebagai kontroler berbasis fungsi biaya adalah satu
...
\begin{table}[H]
    ...
    \hline
    Statik        & 5.288{,}52 & 5.155{,}40 & 335{,}62 & 277{,}33 \\
    \hline
    Rule-Based    & 2.206{,}72 & 2.675{,}07 & 339{,}70 & 360{,}19 \\
    \hline
    PID           &    145{,}60 &   187{,}92 & 228{,}84 & 220{,}34 \\
    \hline
    ANN           &    187{,}55 &   265{,}00 & 206{,}65 & 231{,}94 \\
    \hline
    \textbf{LQR}  & \textbf{143{,}28} & \textbf{173{,}94} & \textbf{179{,}73} & \textbf{164{,}55} \\
    \hline
    \end{tabular}
\end{table}

LQR memberikan cost $J$ terendah di seluruh empat eksperimen, dengan ANN dan PID
bergantian menempati posisi kontroler adaptif terbaik kedua antar konfigurasi mesin. Statik dan Rule-Based mengalami lonjakan drastis pada
mesin Intel, sekitar 15 hingga 30 kali lebih tinggi dibanding AMD, yang menunjukkan
bahwa strategi dengan parameter tetap sangat rentan terhadap perbedaan karakteristik
perangkat keras.
PID, LQR, dan ANN jauh lebih stabil lintas mesin karena ketiganya bereaksi terhadap
kondisi aktual sistem. ANN dan LQR sama-sama \emph{Q-aware} sehingga perilaku keduanya
bergeser saat preset $Q$ berubah, sementara PID memberikan respon yang sama terlepas
dari prioritas yang dipilih.
